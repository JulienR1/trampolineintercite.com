...
type FilterRoute<
    ...
    RequiredModifiers extends
        | readonly RouteModifer[]
        | undefined = R["modifiers"],
> = RequiredModifiers extends readonly [
    infer FirstRequiredModifier extends RouteModifer,
    ...infer RestOfRequiredModifiers extends RouteModifer[],
]
...
export type FilterRoutes<
    R extends RouteArray,
    M extends readonly RouteModifer[],
> = R extends readonly [
    infer First extends Route,
    ...infer Rest extends RouteArray,
]
    ? [FilterRoute<First, M>, ...FilterRoutes<Rest, M>]
    : [];

export const filter = <R extends RouteArray, M extends readonly RouteModifer[]>(
    allowed: M,
    toFilter: R,
): FilterRoutes<R, M> =>
    toFilter
        .filter((route) =>
            (route.modifiers ?? []).every((modifier) =>
                allowed.includes(modifier),
            ),
        )
        .map((route) =>
            "subroutes" in route
                ? { ...route, subroutes: filter(allowed, route.subroutes!) }
                : route,
        ) as FilterRoutes<R, M>;
...
export type ListRoutes<R extends RouteArray> = R extends readonly [
    infer First extends Route,
    ...infer Rest extends RouteArray,
]
    ?
